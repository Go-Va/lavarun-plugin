...
public class AxisAlignedBB {
    ...
    public static AxisAlignedBB fromLocation(Location min, Location max) {
    ...
    public List<Location> toLocations(World world) {
    ...
    public AxisAlignedBB getIntersection(AxisAlignedBB bb) {
        double newX = Math.max(this.minX, bb.minX);
        double newY = Math.max(this.minY, bb.minY);
        double newZ = Math.max(this.minZ, bb.minZ);



        double newWidth = Math.min(this.minX + getWidth(), bb.minX + bb.getWidth()) - newX;
        double newHeight = Math.min(this.minY + this.getHeight(), bb.minY + bb.getHeight()) - newY;
        double newLength = Math.min(this.minZ + this.getLength(), bb.minZ + bb.getLength()) - newZ;

        if (newWidth <= 0d || newHeight <= 0d || newLength <= 0d) return null;

        return new AxisAlignedBB(newX, newY, newZ, newWidth, newHeight, newLength);
    }
